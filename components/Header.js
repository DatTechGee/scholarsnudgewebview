...
const breadcrumbMap = {
  ...
  '/student/chat': { label: 'Messages', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
...
export default function Header({ user, onMenuToggle }) {
  ...
  return (
    <header className="h-14 sm:h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-white border-b border-surface-200/60 shrink-0 shadow-soft sticky top-0 z-30">
      <div className="flex items-center gap-2 sm:gap-3">
        {/* Mobile menu button */}
        {onMenuToggle && (
          <button
            onClick={onMenuToggle}
            className="p-2 rounded-xl hover:bg-surface-100 text-surface-500 hover:text-surface-700 transition-all md:hidden"
          >
        ...
        {entry?.icon && (
          <div className="hidden sm:flex w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500/10 to-secondary-500/10 items-center justify-center">
        ...
        <div>
          <h1 className="text-base sm:text-lg font-bold text-surface-800">{label}</h1>
          <div className="hidden sm:flex items-center gap-1.5 text-[11px] text-surface-400 font-medium">
      ...
      {user ? (
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="hidden sm:flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-primary-50 to-secondary-50 border border-primary-200/40 text-primary-700 text-[11px] font-bold uppercase tracking-wider shadow-sm">
            <div className="w-2 h-2 rounded-full bg-accent-500 animate-pulse-soft" />
            {user.role === 'super_admin' ? 'Super Admin' : user.role}
          </div>
          <div className="flex items-center gap-2 sm:gap-3 sm:pl-3 sm:border-l border-surface-200">
            ...
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-primary-500 to-secondary-500 flex items-center justify-center text-white font-extrabold text-xs sm:text-sm shadow-glow ring-2 ring-white">
